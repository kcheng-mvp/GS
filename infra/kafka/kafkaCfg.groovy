...
server {
    listeners = "PLAINTEXT://:${ka.client.port}"
    log.dirs = "/data0/kafka/data"
    num.partitions = 2
    broker.id = ka.hosts.indexOf(currentHost) + 1
    zookeeper.connect = zk.hosts.collect { "${it}:${zk.client.port}" }.join(",") + "/kafka"
}



producer {
    bootstrap.servers = ka.hosts.collect { "${it}:${ka.client.port}" }.join(",")
}
